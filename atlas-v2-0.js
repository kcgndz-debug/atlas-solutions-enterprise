...
location.href=location.origin+location.pathname+'?v=2.0.0';
}catch(e){$('configError').textContent=e.message}};$('changeConfig').onclick=()=>{localStorage.removeItem(K);location.reload()};$('signInTab').onclick=()=>tabs(0);$('ownerTab').onclick=()=>tabs(1);$('signIn').onsubmit=login;$('ownerSetup').onsubmit=owner;$('logout').onclick=()=>signOut(auth);document.querySelectorAll('.nav').forEach(b=>b.onclick=()=>view(b.dataset.view));document.querySelectorAll('.newEstimate').forEach(b=>b.onclick=()=>view('estimate'));$('workspace').onchange=()=>{company=V('workspace');watchData()};$('estType').onchange=type;$('estProject').onchange=projectSelected;$('quoteMode').onchange=calc;$('fieldVerify').onchange=calc;$('estimateForm').addEventListener('input',calc);$('estimateForm').onsubmit=saveEstimate;$('customerForm').onsubmit=saveCustomer;$('projectForm').onsubmit=saveProject;$('companyForm').onsubmit=saveCompany;$('print').onclick=()=>print()}
...
let qc=query(collection(db,'customers'),where('companyId','==',company),orderBy('name'));
subs.push(onSnapshot(qc,s=>{
let a=s.docs.map(d=>({id:d.id,...d.data()}));customers=a;
...
let qp=query(collection(db,'projects'),where('companyId','==',company),orderBy('createdAt','desc'));
...
$('estProject').innerHTML='<option value="">Select project</option>'+projects.map(x=>`<option value="${x.name}">${x.name}</option>`).join('')
}));
}
...
let tax=0,taxFee=0,taxRate=N('salesTaxRate')/100;
if(mode==='material'){
...
quoteNumber,
companyId:company,companyName:companyName(),customer:V('estCustomer'),project:V('estProject'),location:V('estLocation'),
type:p[0],description:p[1],material:p[2],leadWeeks:p[4],status:'draft',
createdBy:user.uid,createdAt:serverTimestamp(),notes:V('estimateNotes'),inputs:{base:N('base'),profit:N('profit'),laborDay:N('laborDay'),mileRate:N('mileRate'),hotel:N('hotel'),miles:N('miles'),nights:N('nights'),travelHalfDays:N('travelHalfDays'),hoops:N('hoops'),posts:N('posts')},...c
});
